...
app.put("/api/foods/:id", verifyToken, async (req, res) => {
  try {
    ...
    const isOwner =
      food.createdBy && food.createdBy.toString() === req.user._id.toString();
    const isAdmin = req.user.role === "admin";
    if (!isOwner && !isAdmin) {
      return res
        .status(403)
        .json({ success: false, message: "无权限修改该美食" });
    }
...
app.delete("/api/foods/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (!isOwner && !isAdmin) {
      return res
        .status(403)
        .json({ success: false, message: "无权限删除该美食" });
    }
...
// 获取今天的推荐美食
app.get("/api/daily-recommendation", verifyToken, async (req, res) => {
  try {
    console.log("🎯 获取今日推荐:", req.user.username);

    // 获取服务器当前日期（北京时间）
    const now = new Date();
    const beijingTime = new Date(now.getTime() + 8 * 60 * 60 * 1000); // UTC+8
    const todayString = beijingTime.toISOString().split("T")[0]; // YYYY-MM-DD格式

    console.log("📅 今日日期:", todayString);
    ...
    if (recommendation && recommendation.foodId) {
      console.log("✅ 找到今日推荐:", recommendation.foodId.name);
      return res.json({
        success: true,
        food: recommendation.foodId,
        message: `今天推荐您尝试：${recommendation.foodId.name} ${recommendation.foodId.emoji}`,
        isNewRecommendation: false,
      });
    }

    // 如果没有推荐，生成新的推荐
    console.log("🎲 生成新的今日推荐...");
    ...
    if (allFoods.length === 0) {
      return res.status(404).json({
        success: false,
        message: "暂无美食数据，请先添加一些美食！",
      });
    }

    // 获取用户历史推荐记录，避免重复推荐（最近7天）
    const sevenDaysAgo = new Date(
      beijingTime.getTime() - 7 * 24 * 60 * 60 * 1000
    );
    ...
    console.log("📝 最近7天推荐过的美食:", recentFoodIds.length, "个");

    // 过滤掉最近推荐过的美食
    let availableFoods = allFoods.filter(
      (food) => !recentFoodIds.includes(food._id.toString())
    );

    // 如果所有美食都被推荐过了，那就从全部美食中选择
    if (availableFoods.length === 0) {
      console.log("🔄 所有美食都推荐过了，重新从全部美食中选择");
      availableFoods = allFoods;
    }

    // 基于用户ID和日期生成种子，确保同一天同一用户得到相同的推荐
    const seed = req.user._id.toString() + todayString;
    const seedHash = Array.from(seed).reduce((a, b) => {
      a = (a << 5) - a + b.charCodeAt(0);
      return a & a;
    }, 0);

    // 使用种子选择美食
    const randomIndex = Math.abs(seedHash) % availableFoods.length;
    const selectedFood = availableFoods[randomIndex];

    console.log("🎯 选中的美食:", selectedFood.name);

    // 保存推荐记录
    const newRecommendation = new DailyRecommendation({
      userId: req.user._id,
    ...
    res.json({
      success: true,
      food: selectedFood,
      message: `今天推荐您尝试：${selectedFood.name} ${selectedFood.emoji}`,
    ...
    res.status(500).json({
      success: false,
      message: "获取今日推荐失败，请稍后重试",
    });
  }
});

// 获取用户的推荐历史
app.get("/api/daily-recommendation/history", verifyToken, async (req, res) => {
  try {
    console.log("📚 获取推荐历史:", req.user.username);
    ...
    const total = await DailyRecommendation.countDocuments({
      userId: req.user._id,
    });
...
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || "http://127.0.0.1:8000")
  .split(",")
  .map((o) => o.trim());
